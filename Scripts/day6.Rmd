...
```{r}
field_irrigated <- read_delim('../Data/spatial_data/wwc5_wells.txt') %>%
...
boundary_kansas <- usa_sf() %>% 
  filter(name=='Kansas')

ggplot() + 
  geom_sf(data = boundary_kansas) +
  geom_sf(data = streams,color='blue') + 
  geom_sf(data = irrigation, color='black', size=0.1)
```
...
#st_difference(high_plain_groundwater,boundary_kansas)
```
```{r}
ggplot() + 
  geom_sf(data = boundary_kansas) +
  ...
  geom_sf(data = irrigation, color='black', size=0.1,alpha=0.4)
```
...
res <- NbClust(df_climate_vars[,1:4], 
               diss = NULL, distance = 'euclidean', min.nc = 2, max.nc = 5, 
               method = "kmeans", index = 'all')

number_optimal <- fviz_nbclust(res, method = "silhouette", print.summary = TRUE) +
  theme_minimal()
...
```{r}
set.seed(123)
km.res <- kmeans(df_climate_vars[,1:4], 6, nstart = 25)

climate_clusters_ks <- cbind(df_climate_vars, as.factor(km.res$cluster))
colnames(kmeans_3Z) <- c('X','Y','cluster')
