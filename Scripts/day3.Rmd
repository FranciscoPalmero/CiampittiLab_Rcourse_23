...
```{r warning=FALSE}
crop_info <- readxl::read_excel("../CiampittiLab_Rcourse_23/Data/N_initiative/Yield_Plant_Measurements.xlsx",
                   sheet = 2) %>% 
...
```{r}
site_info <- readxl::read_excel("../CiampittiLab_Rcourse_23/Data/N_initiative/1.Site_Characterization.xlsx",
                   sheet = 2) %>% 
  rename(`Trial#` = `Trail#`) %>% 
  mutate(across(where(is.character), 
                ~na_if(., "."))) %>% 
...
```{r warning=FALSE}
irrig_info <- readxl::read_excel("../CiampittiLab_Rcourse_23/Data/N_initiative/11.Irrigation.xlsx",
                   sheet = 2) %>% 
  mutate(across(where(is.character), 
                ~na_if(., "."))) %>% 
